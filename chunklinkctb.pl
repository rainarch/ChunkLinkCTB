...
###
### Produces a convenient list of words, chunks, functions, and links from Penn Chinese TreeBank 4
###
### 
### Modified by Wenliang Chen, NiCT, chenwl@nict.go.jp or chenwenliang@gmail.com, 2005
###
...
sub help {
print "call as:                                                                                      \n";
print "  chunklink_ctb.pl <options> /cdrom/ctb4/data/bracketed/chtb_????.fid | more                  \n";
print "                                                                                              \n";
print "options:                                                                                      \n";
...
##CTB4 CLP will be pruned after QP
$prune_if_infrontof_head{'QP'}{'CLP'}=1;
$prune_if_infrontof_head{'DP'}{'CLP'}=1;
...
foreach $file (@ARGV) {                     # loop: for each treebank file
    ...
    if ($file =~ /\/chtb_([0-9]+)\.fid$/) {  # extract file number
	$filenumber=$1;
    }
    else {
	$filenumber=1;#die "$file does not match!\n";
    }
...
sub print_flatten {
	...
	if ($opt_i==0) { 
	    ...
	    # ++ new case
	    # PU as B-NP or B-VP but the next tag is B-XP
	    # --> O
	    if(    $flattened[$i]->{pos_tag} eq "PU" 
		&& $flattened[$i]->{iob_tag_inner}=~ /^B-(.*)$/
		&& ($i + 1 == @flattened || $flattened[$i +1]->{iob_tag_inner}=~ /^B-(.*)$|^C-(.*)$|^O$/)){
	  	$flattened[$i]->{iob_tag_inner}="O";
	    }
	    # ++ new case
	    # PU + PU as B-XP + I-XP, but the next tag is O		
	    # --> O + O
	    if(    $flattened[$i]->{pos_tag} eq "PU" 
		&& $flattened[$i]->{iob_tag_inner}=~ /^B-(.*)$/
		&& $i + 1 < @flattened
		&& $flattened[$i+1]->{pos_tag} eq "PU" 
		&& $flattened[$i+1]->{iob_tag_inner}=~ /^I-(.*)$|^E-(.*)$/){
		$flattened[$i]->{iob_tag_inner}="O";
		$flattened[$i+1]->{iob_tag_inner}="O";
	    }	
...
#RP=DEC|DEG|DER|DEV|SP|AS|ETC|SP|MSP
$head{'ADJP'}='JJ|AD|VA|VC|VE|VV|CS|P|LC|IJ|FW|DEC|DEG|DER|DEV|SP|AS|ETC|SP|MSP|DT|NN|NR|NT';
# (ADVP (UH Indeed))
#$head{'ADVP'}='RB|IN|TO|DT|PDT|JJ|RP|FW|LS|UH|CC|NN|CD|VB';
$head{'ADVP'}='AD|CS|P|LC|DT|JJ|DEC|DEG|DER|DEV|SP|AS|ETC|SP|MSP|FW|IJ|CC|NN|NR|NT|CD|OD|VA|VC|VE|VV';
$head{'CLP'}='M'; 
$head{'DP'}='DT|M'; 
$head{'DNP'}='DEG'; 
$head{'DVP'}='DEV'; 
...
#$head{'FRAG'}='[A-Z]'; 
$head{'LCP'}='LC'; 
$head{'LST'}='CD|PU|FW|OD';
#$head{'NP'}='NN|CD|PRP|JJ|DT|EX|IN|RB|VB|FW|SYM|UH|WP|WDT';
$head{'NP'}='NN|NR|NT|CD|OD|JJ|OD|DT|CS|P|LC|AD|VA|VC|VE|VV|FW|PU|ETC|M|P';
#$head{'PP'}='IN|TO|RB|VBG|VBN|JJ|RP|CC|FW';
$head{'PP'}='P|CS|P|LC|AD|VA|VC|VE|VV|JJ|OD|DEC|DEG|DER|DEV|SP|AS|ETC|SP|MSP|CC|FW|';
#$head{'PRN'}= '[A-Z]';
# "第一" 
$head{'QP'}='CD|DT|OD|JJ|M|NN|NR|NT|PU';
#$head{'UCP'}='JJ|NN|VB|CD|NR|NT|VA|VC|VE|VV';
# AS了|DER得|SP了|BA把|LBlong Bei被|SBShort Bei被
#$head{'VP'}='VB|MD|TO|JJ|NN|POS|FW|SYM';
$head{'VP'}='VA|VC|VE|VV|AS|DER|SP|BA|LB|SB|JJ|OD|FW|NN|NR|NT';
##
$head{'WHADJP'}='JJ';
$head{'WHADVP'}='WRB|IN|RB|WDT';
$head{'WHNP'}='NN|NR|NT|PU|ETC|VA|VC|VE|VV|CD|JJ|OD|DT|FW|M|P'; # including WP$
$head{'WHPP'}='P';
